...
async function createDatapackZip(id, commands) {
	...
	fs.writeFileSync(
		...
		JSON.stringify({ pack: { pack_format: 18, description: "Minecraft Image Bot Datapack" } })
	);
	...
	fs.writeFileSync(
		path.join(folder, "data", "minecraft", "tags", "functions", "load.json"),
		JSON.stringify({ values: ["image:welcome"] })
	);
...
async function getCommandPath(id) {
	...
	commands.push(
		`tellraw @s [{"text":"Placing blocks... This could take a while...\\n","color":"green"},{"text":"It is recommended that you turn ", "color":"aqua"},{"text":"OFF", "bold":true, "color":"red"},{"text":" Smooth Lighting","color":"aqua"},{"text":"\\nThis can be done from:","color":"aqua"},{"text":" Options > Video Settings > Smooth Lighting", "color":"gold"}]`
	);

	for (block of json.blocks) {
		commands.push(`setblock ~${block.pos.x} ~-1 ~${block.pos.y} dirt`);
		if (block.block === "dried_kelp")
			commands.push(`setblock ~${block.pos.x} ~ ~${block.pos.y} ${block.block}_block`);
		else if (block.block === "piston_sticky")
			commands.push(`setblock ~${block.pos.x} ~ ~${block.pos.y} sticky_piston[facing=up]`);
		else if (block.block === "piston")
			commands.push(`setblock ~${block.pos.x} ~ ~${block.pos.y} piston[facing=up]`);
		else if (block.block === "farmland_moist")
			commands.push(`setblock ~${block.pos.x} ~ ~${block.pos.y} farmland[moisture=7]`);
		else if (block.block === "mushroom_block")
			commands.push(`setblock ~${block.pos.x} ~ ~${block.pos.y} brown_mushroom_block`);
